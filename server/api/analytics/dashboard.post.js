export default defineEventHandler(async (event) => {
    ...
    const dateFilter = timeRange === 'last7days' 
      ? `WHERE transaction_date >= NOW() - INTERVAL '7 days' AND user_id = $1`
      : timeRange === 'last30days'
        ? `WHERE transaction_date >= NOW() - INTERVAL '30 days' AND user_id = $1`
        : timeRange === 'last90days' 
          ? `WHERE transaction_date >= NOW() - INTERVAL '90 days' AND user_id = $1`
          : 'WHERE user_id = $1'
